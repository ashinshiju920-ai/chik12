...
export const ShopCatalogView: React.FC = () => {
  ...
  return (
    <div ref={catalogTopRef} className="bg-[#F9F8F6] min-h-screen pb-20">
      
      {/* ========================================================= */}
      {/* 1. HEADER & SUB-CATEGORY PILL BAR                         */}
      {/* ========================================================= */}
      <div className="border-b border-[#EAE6DE] bg-white/60 backdrop-blur-md sticky top-16 z-30 transition-all">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          
          {/* Breadcrumbs */}
          <nav className="text-xs text-neutral-400 tracking-wide font-sans mb-1.5 flex items-center gap-1.5 flex-wrap">
            <button 
            ...
            <span className="text-neutral-300">/</span>
            <span className="text-neutral-900 font-medium">{activeCategoryName}</span>
          </nav>

          {/* Title & Count Row */}
          <div className="flex flex-col sm:flex-row sm:items-baseline justify-between gap-1 mb-3">
            <h1 className="text-2xl sm:text-3xl font-semibold tracking-tight text-neutral-900 font-heading font-serif">
              {activeCategoryName}
            </h1>
            <span className="text-xs text-neutral-500 font-sans">
              Showing <span className="font-semibold text-neutral-900">{filteredProducts.length}</span> curated pieces
            </span>
          </div>
              ...
              return (
                <button
                  ...
                  className={`relative px-4 py-1.5 rounded-full border text-xs font-medium whitespace-nowrap transition-all duration-200 cursor-pointer flex items-center gap-1.5 shrink-0 ${
                    isSelected
                      ? 'text-white border-neutral-900 shadow-xs'
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-6">
        ...
        {/* ========================================================= */}
        <div className="lg:hidden flex items-center justify-between gap-3 mb-6 bg-white border border-[#EAE6DE] p-3 rounded-xl shadow-xs">
          <button
            onClick={() => setIsMobileFilterOpen(true)}
            className="flex-1 bg-neutral-900 text-white text-xs font-semibold py-2.5 px-4 rounded-lg flex items-center justify-center gap-2 cursor-pointer shadow-xs transition-transform active:scale-98"
          >
        ...
        {activeFiltersCount > 0 && (
          <div className="flex flex-wrap items-center gap-2 mb-6 pb-4 border-b border-[#EAE6DE]">
            <span className="text-xs font-semibold text-neutral-700">Active Filters:</span>

            {selectedCategory !== 'all' && (
              <span className="inline-flex items-center gap-1.5 text-xs bg-white border border-[#D5D0C5] text-neutral-800 px-3 py-1 rounded-full shadow-2xs">
                <span>{activeCategoryName}</span>
                <button onClick={() => setSelectedCategory('all')} className="text-neutral-400 hover:text-neutral-900 cursor-pointer">
                  <X className="w-3 h-3" />
                </button>
              </span>
            )}
            ...
            {searchQuery && (
              <span className="inline-flex items-center gap-1.5 text-xs bg-white border border-[#D5D0C5] text-neutral-800 px-3 py-1 rounded-full shadow-2xs">
                ...
                <button onClick={() => setSearchQuery('')} className="text-neutral-400 hover:text-neutral-900 cursor-pointer">
                  <X className="w-3 h-3" />
                </button>
              </span>
            )}
            ...
            {priceMax < 300 && (
              <span className="inline-flex items-center gap-1.5 text-xs bg-white border border-[#D5D0C5] text-neutral-800 px-3 py-1 rounded-full shadow-2xs">
                ...
                <button onClick={() => setPriceMax(300)} className="text-neutral-400 hover:text-neutral-900 cursor-pointer">
                  <X className="w-3 h-3" />
                </button>
              </span>
            )}
            ...
            {inStockOnly && (
              <span className="inline-flex items-center gap-1.5 text-xs bg-white border border-[#D5D0C5] text-neutral-800 px-3 py-1 rounded-full shadow-2xs">
                ...
                <button onClick={() => setInStockOnly(false)} className="text-neutral-400 hover:text-neutral-900 cursor-pointer">
                  <X className="w-3 h-3" />
                </button>
              </span>
            )}
            ...
            {onSaleOnly && (
              <span className="inline-flex items-center gap-1.5 text-xs bg-white border border-[#D5D0C5] text-neutral-800 px-3 py-1 rounded-full shadow-2xs">
                <span>On Sale</span>
                <button onClick={() => setOnSaleOnly(false)} className="text-neutral-400 hover:text-neutral-900 cursor-pointer">
                  <X className="w-3 h-3" />
                </button>
              </span>
            )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          <aside className="hidden lg:block lg:col-span-3 sticky top-36 space-y-6">
            ...
            <div className="bg-white border border-[#EAE6DE] rounded-xl p-5 space-y-4 shadow-xs">
              <div className="flex items-center justify-between pb-3 border-b border-[#F0ECE1]">
                <h3 className="text-xs font-bold text-neutral-900 uppercase tracking-wider font-sans">
                  Departments
                </h3>
                <span className="text-[11px] text-neutral-400 font-mono">{departments.length}</span>
              </div>
            ...
            {/* Price Filter Box */}
            <div className="bg-white border border-[#EAE6DE] rounded-xl p-5 space-y-4 shadow-xs">
              <div className="flex justify-between items-center text-xs font-bold text-neutral-900 uppercase tracking-wider font-sans">
                <span>Max Price</span>
            ...
            <div className="bg-white border border-[#EAE6DE] rounded-xl p-5 space-y-3.5 shadow-xs">
              <h3 className="text-xs font-bold text-neutral-900 uppercase tracking-wider font-sans pb-2 border-b border-[#F0ECE1]">
                Sort Curations
              </h3>
                ...
                {[
                  ...
                  <button
                    ...
                    className={`w-full flex items-center justify-between text-xs py-1.5 px-2 rounded-md transition-colors cursor-pointer text-left ${
                      sortBy === s.value ? 'font-bold text-neutral-900 bg-neutral-100' : 'text-neutral-600 hover:text-neutral-900'
                    }`}
                  >
                    <span>{s.label}</span>
                    {sortBy === s.value && <Check className="w-3.5 h-3.5 text-neutral-900" />}
                  </button>
                ))}
              </div>
              ...
              <div className="pt-3 border-t border-[#F0ECE1] space-y-2">
                ...
                <label className="flex items-center gap-2.5 text-xs text-neutral-800 cursor-pointer">
                  <input
                    ...
                    className="rounded text-[#C85A32] focus:ring-[#C85A32] cursor-pointer"
                  />
          ...
          <main className="lg:col-span-9 space-y-8">
            {filteredProducts.length === 0 ? (
              <div className="bg-white border border-[#EAE6DE] rounded-2xl p-16 text-center space-y-4 shadow-xs">
                <div className="w-16 h-16 rounded-full bg-[#F5F3EF] flex items-center justify-center text-neutral-400 mx-auto">
                ...
                <button
                  onClick={handleResetFilters}
                  className="bg-neutral-900 hover:bg-neutral-800 text-white text-xs font-semibold px-6 py-3 rounded-xl transition-colors cursor-pointer"
                >
                  Clear All Filters
                </button>
              </div>
              ...
              <>
                ...
                {/* ========================================================= */}
                {totalPages > 1 && (
                  <div className="pt-10 pb-4 border-t border-[#EAE6DE] flex flex-col sm:flex-row items-center justify-between gap-4">
                    
                    {/* Catalog Range Indicator */}
                    <span className="text-xs text-neutral-500 font-sans order-2 sm:order-1">
                      Showing <span className="font-semibold text-neutral-900">{(safeCurrentPage - 1) * itemsPerPage + 1}–{Math.min(safeCurrentPage * itemsPerPage, filteredProducts.length)}</span> of <span className="font-semibold text-neutral-900">{filteredProducts.length}</span> pieces
                    </span>
                    ...
                    <nav className="flex items-center gap-1.5 order-1 sm:order-2" aria-label="Catalog pagination">
                      ...
                      <button
                        ...
                        className="px-3 py-1.5 rounded-full border border-neutral-200 text-xs font-medium text-neutral-700 hover:border-neutral-900 hover:text-neutral-900 transition-colors flex items-center gap-1 disabled:opacity-30 disabled:pointer-events-none cursor-pointer"
                      ...
                      <div className="flex items-center gap-1">
                        {paginationRange.map((page, index) => {
                          if (page === '...') {
                            return (
                              <span key={`ellipsis-${index}`} className="w-8 h-8 flex items-center justify-center text-xs text-neutral-400">
                                ...
                              </span>
                            );
                          }
                            ...
                            <button
                              ...
                              className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-medium transition-all cursor-pointer ${
                                isActive
                                  ...
                                  : 'text-neutral-600 hover:text-neutral-900 hover:bg-white border border-transparent hover:border-neutral-200'
                              }`}
                            >
                              {pageNum}
                            </button>
